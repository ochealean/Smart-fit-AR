...
function showShopModal(e) {
    ...
    const unsubscribe = readDataRealtime(shopPath, (result) => {
        if (result.success) {
            ...
            // Unsubscribe after first load to avoid multiple listeners
            unsubscribe();
        } else {
...
function showConfirmationDialog(e, actionType) {
    ...
    // Use readDataRealtime to get shop data
    const unsubscribe = readDataRealtime(shopPath, (result) => {
        if (result.success) {
            const shop = result.data;
...
function initializeEventListeners() {
    ...
    // CHANGE THIS VARIABLE NAME to avoid conflict with the imported function
    const updatePayload = {  // ← Renamed from updateData to updatePayload
    ...
    // Now this will call the imported updateData function correctly
    updateData(shopPath, updatePayload)  // ← updateData is the imported function
        .then((result) => {
            if (result.success) {
                ...
                if (currentAction === "reject") {
                    ...
                    const templateParams = {
                        ...
                        message: reason, // Use the rejection reason
